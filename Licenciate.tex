...
P_{\mu\nu}(\rho) = \llangle[\Big]  \Phi_{\mu}(\rho) \Big\lvert \frac{\partial}{\partial\rho} \Big\rvert  \Phi_{\nu}(\rho)  \rrangle[\Big],
\end{equation}
and

\begin{equation}\label{couplingQ}
Q_{\mu\nu}(\rho) = \llangle[\Big]  \Phi_{\mu}(\rho) \Big\lvert \frac{\partial^2}{\partial\rho^2} \Big\rvert  \Phi_{\nu}(\rho)  \rrangle[\Big],
\end{equation}
where the double brackets denote integration over the two angular coordinates. The completeness of the channelfunctions leads to the following relationship between $P_{\mu \nu}$ and $Q_{\mu \nu}$
...
\begin{equation}
P^2_{\mu \nu} = \sum_{\sigma}P_{\mu \sigma}P_{\sigma \nu} = - \llangle[\Big] \frac{\partial \Phi_{\mu}(\rho)}{\partial \rho}  \Big\lvert \frac{\partial \Phi_{\nu}(\rho)}{\partial\rho}  \rrangle[\Big].
\end{equation}
The coupling matrix $P_{\mu \nu}$ is antisymmetric, i.e. $P_{\mu \nu}= - P_{\nu \mu}$.

\subsection{Generalized Hellmann-Feynman theorem}
The following subsection concerns the mathematical description of the nonadiabatic coupling matrices that was defined in subsection \ref{sec:AHM}. To obtain an expression for the coupling term $P_{\mu \nu}$ given in \eqref{couplingP}, a generalized Hellmann-Feynman (HF) approach can be used. Consider $\nu$ adiabatic eigenstates $\Phi_{\nu}(\rho;\theta,\phi)=\Phi_{\nu}(\rho;\Omega)$, which are obtained by
\begin{equation}\label{HA}
H_{a}(\rho,\Omega)\Phi_{\nu}(\rho;\Omega) = U_{\nu}(\rho)\Phi_{\nu}(\rho;\Omega),
\end{equation}
where the adiabatic Hamiltonian $H_a$ is just the hyperangular part of the Schr{\"o}dinger equation given in \eqref{adiabatic}, the eigenvalues $U_{\nu}(\rho)$ are the effective adiabatic three-body potential curves and $\Phi_{\nu}$. Using the identities 

\begin{equation}
\llangle[\big]  \Phi_{\mu}(\rho;\Omega)  \big\rvert  \Phi_{\nu}(\rho;\Omega)  \rrangle[\big] = \delta_{\mu \nu}
\end{equation}
and 

\begin{equation}
\frac{\partial}{\partial \rho}\llangle[\big]  \Phi_{\nu}(\rho;\Omega)  \big\rvert  \Phi_{\nu}(\rho;\Omega)  \rrangle[\big] = 0,
\end{equation}
we start by deriving the diagonal part of the HF theorem. Multiply with $\Phi_{\nu}^{\dagger}(\rho;\Omega)$ to the left on both sides of \eqref{HA} and integrate over the two hyperangles $\Omega$. Differentiating with respect to the hyperradius then gives

\begin{align}
\frac{\partial U_{\nu}(\rho)}{\partial \rho}&=\frac{\partial}{\partial \rho}\llangle[\big]  \Phi_{\nu}(\rho;\Omega)  \big\lvert H_a(\rho,\Omega) \big\rvert  \Phi_{\nu}(\rho;\Omega)  \rrangle[\big]\nonumber\\
&=\llangle[\Big]  \Phi_{\nu}(\rho;\Omega)  \Big\lvert \frac{\partial H_a(\rho,\Omega)}{\partial \rho} \Big\rvert  \Phi_{\nu}(\rho;\Omega)  \rrangle[\Big] +  \llangle[\Big]  \Phi_{\nu}(\rho;\Omega)  \Big\lvert H_a(\rho,\Omega)\Big\rvert  \frac{\partial \Phi_{\nu}(\rho;\Omega)}{\partial \rho}  \rrangle[\Big] \nonumber \\
&+\llangle[\Big] \frac{\partial \Phi_{\nu}(\rho;\Omega)}{\partial \rho}    \Big\lvert H_a(\rho,\Omega)\Big\rvert  \Phi_{\nu}(\rho;\Omega)  \rrangle[\Big] \nonumber \\
&=\llangle[\Big]  \Phi_{\nu}(\rho;\Omega)  \Big\lvert \frac{\partial H_a(\rho,\Omega)}{\partial \rho} \Big\rvert  \Phi_{\nu}(\rho;\Omega)  \rrangle[\Big] + U_{\nu}(\rho)\llangle[\Big]  \Phi_{\nu}(\rho;\Omega) \Big\rvert  \frac{\partial \Phi_{\nu}(\rho;\Omega)}{\partial \rho}  \rrangle[\Big] \nonumber \\
&+\llangle[\Big] \frac{\partial \Phi_{\nu}(\rho;\Omega)}{\partial \rho}\Big\rvert  \Phi_{\nu}(\rho;\Omega)  \rrangle[\Big] \nonumber \\
&=\llangle[\Big]  \Phi_{\nu}(\rho;\Omega)  \Big\lvert \frac{\partial H_a(\rho,\Omega)}{\partial \rho} \Big\rvert  \Phi_{\nu}(\rho;\Omega)  \rrangle[\Big] + U_{\nu}(\rho) \frac{\partial}{\partial \rho}\llangle[\Big]  \Phi_{\nu}(\rho;\Omega) \Big\rvert \Phi_{\nu}(\rho;\Omega)  \rrangle[\Big] \nonumber \\
&=\llangle[\Big]  \Phi_{\nu}(\rho;\Omega)  \Big\lvert \frac{\partial H_a(\rho,\Omega)}{\partial \rho} \Big\rvert  \Phi_{\nu}(\rho;\Omega)  \rrangle[\Big].
\end{align}
The generalized HF theorem also includes the nondiagonal terms. Multiplying with $\Phi_{\mu}^{\dagger}(\rho;\Omega)$ to the left on both sides of \eqref{HA} and integrate over the two hyperangles $\Omega$, followed by differentiation with respect to the hyperradius now gives

\begin{align}
&\frac{\partial U_{\nu}(\rho)}{\partial \rho}\llangle[\Big]  \Phi_{\mu}(\rho;\Omega)  \Big\lvert \Phi_{\nu}(\rho;\Omega)  \rrangle[\Big] + U_{\nu}\llangle[\Big]  \Phi_{\mu}(\rho;\Omega)  \Big\lvert \frac{\partial \Phi_{\nu}(\rho;\Omega) }{\partial \rho} \rrangle[\Big]\nonumber\\
&+U_{\nu}\llangle[\Big] \frac{\partial \Phi_{\mu}(\rho;\Omega) }{\partial \rho}  \Big\lvert \Phi_{\nu}(\rho;\Omega)  \rrangle[\Big] = \llangle[\Big]  \Phi_{\mu}(\rho;\Omega)  \Big\lvert \frac{\partial H_a(\rho,\Omega)}{\partial \rho} \Big\rvert  \Phi_{\nu}(\rho;\Omega)  \rrangle[\Big] \nonumber \\
&+ U_{\mu}(\rho)\llangle[\Big]  \Phi_{\mu}(\rho;\Omega) \Big\rvert  \frac{\partial \Phi_{\nu}(\rho;\Omega)}{\partial \rho}  \rrangle[\Big]+U_{\nu}\llangle[\Big] \frac{\partial \Phi_{\mu}(\rho;\Omega)}{\partial \rho}\Big\rvert  \Phi_{\nu}(\rho;\Omega)  \rrangle[\Big]. 
\end{align}
The first term on the left side of this equation i zero by the orthogonality of the channel functions. By removing the last term on both sides we finally obtain the expression

\begin{equation}\label{derivaham}
\llangle[\bigg]  \Phi_{\mu}(\rho) \bigg\lvert \frac{\partial H_{ad}(\rho,\Omega)}{\partial \rho} \bigg\rvert \Phi_{\nu}(\rho) \rrangle[\bigg]= \big(U_{\nu}(\rho)-U_{\mu}(\rho)\big) \llangle[\bigg]  \Phi_{\mu}(\rho) \bigg\lvert \frac{\partial}{\partial \rho} \bigg\rvert \Phi_{\nu}(\rho) \rrangle[\bigg],
\end{equation}
and we can express the nonadiabatic coupling terms $P_{\mu \nu}$ as

\begin{equation}
P_{\mu \nu} = \frac{1}{\big(U_{\nu}(\rho)-U_{\mu}(\rho)\big)}\llangle[\bigg]  \Phi_{\mu}(\rho) \bigg\lvert \frac{\partial H_{ad}(\rho,\Omega)}{\partial \rho} \bigg\rvert \Phi_{\nu}(\rho) \rrangle[\bigg].
\end{equation}
...
\begin{equation}
\frac{\partial U_{\nu}(\rho)}{\partial \rho} = \sum_{ij}(C_{\nu}^i)^{\dagger}C_{\nu}^j \frac{\partial H_{ij}}{\partial \rho}.
\end{equation}
In the non-diagonal case the result in matrical form reads 

\begin{equation}
\llangle[\bigg]  \Phi_{\mu}(\rho) \bigg\lvert \frac{\partial}{\partial \rho} \bigg\rvert \Phi_{\nu}(\rho) \rrangle[\bigg] = P_{\mu \nu} = \sum_{ij}(C_{\mu}^i)^{\dagger}C_{\nu}^j \frac{\partial H_{ij}}{\partial \rho}.
\end{equation}
...
The first step in solving the adiabatic Hamiltonian given by \eqref{adiabatic}, is to expand the solution, i.e. the channel functions $\Phi_{\nu}$, in a suitable basis, such as B-splines. Make the ansatz

\begin{equation}\label{expansion}
\Phi_{\nu}(\rho;\theta,\phi) = \sum_{l,m}^{L,M} c^{l,m}_{\nu}B_{l,k_l}(\theta)B_{m,k_m}(\phi)).
\end{equation}
The channelfunctions are now characterized by the $LM$ expansion coefficients. The upper limits of this sum is determined by the number of mesh points, the order of the  B-splines and the number of conditions (n.o.c.) on the coordinate in question. If we construct a mesh containing our physical points with $N_{\theta}$ points in the $\theta$-direction and $N_{\phi}$ points in the $\phi$-direction, then the number of B-splines needed for each coordinate is given by $L = N_{\theta}+k-\text{n.o.c}$ and $M = N_{\phi}+k-\text{n.o.c}$. For a second order partial differential equation the B-splines must be twice differentiable everywhere on the knot sequence and have continuous second order derivatives. With the B-spline definitions given in \ref{B-splines}, the above requirements is fullfilled for B-splines of order $k\geq 4$. The second step in this approach is \textit{collocation}. Collocation means that we require the equation, in this case equation \eqref{adiabatic}, to be fullfilled in specific points. The endpoints for each coordinate interval cointans $(k-1)$ ghost points. dimensions with $N$ physical points is considered, we will put $k$ knotpoints in the first  and last physical point. there will be   

\begin{equation}\label{expansion}
\Phi_{\nu}(\rho;\theta_i,\phi_i) = \sum_{l_i}^{L_i}\sum_{m_i}^{M_i} c^{l_i,m_i}_{\nu}B_{l_i,k_l}(\theta_i)B_{m_i,k_m}(\phi_i)).
\end{equation} 

substituting $\Phi_{\nu}$ from equation \eqref{expansion} into \eqref{adiabatic} and projecting out $B_{l_i,k_l}(\theta_i)B_{m_i,k_m}(\phi_i))$
\begin{equation}
N
\end{equation}
