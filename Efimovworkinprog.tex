...
Some claim \cite{Efimov:1970zz}

\subsection{The Three-body Problem}
The $N$-body problem is a class of problems in physics which, in a most general sense, consists of modelling the motion of $N$ objects, interacting through some physical force. In classical mechanics the general equations of motion for point masses are derived from Newton's second law of motion, which states that the rate of change of momentum for an object equals the force acting on it. The problem can be defined as follows. Consider a system of $N$ point-like particles in $\mathbb{R}^{3}$ with masses $m_{i}$, position vectors $\mathbf{q}_{i}$, and momenta $\mathbf{p}_{i} = m_{i} \frac{d \mathbf{q}_{i}}{dt}$ where $i = 1, \ldots , N$ respectively.  
